...
func TestEntries_ReverseAndList(t *testing.T) {
	...
	h.ServeHTTP(rec, req)
	if rec.Code != http.StatusCreated {
		t.Fatalf("create entry expected 201, got %d", rec.Code)
	}
	var er entryResp
	...
	if rec2.Code != http.StatusCreated {
		t.Fatalf("reverse expected 201, got %d: %s", rec2.Code, rec2.Body.String())
	}
	var er2 entryResp
	_ = json.Unmarshal(rec2.Body.Bytes(), &er2)
	if len(er2.Lines) != 2 {
		t.Fatalf("expected 2 lines in reversal")
	}
	...
	if rec3.Code != http.StatusOK {
		t.Fatalf("list entries expected 200, got %d", rec3.Code)
	}
}
...
func TestReclassify_BlockedAfterReverse(t *testing.T) {
	...
	if rr1.Code != http.StatusCreated {
		t.Fatalf("reverse expected 201, got %d", rr1.Code)
	}
	...
	if rr2.Code != http.StatusUnprocessableEntity {
		t.Fatalf("expected 422, got %d: %s", rr2.Code, rr2.Body.String())
	}
	var eresp errResp
	_ = json.Unmarshal(rr2.Body.Bytes(), &eresp)
	if eresp.Code != "already_reversed" {
		t.Fatalf("expected already_reversed, got %q", eresp.Code)
	}
}
...
func TestEntries_IdempotencyHeader(t *testing.T) {
	...
	var e1 entryResp
	_ = json.Unmarshal(rr1.Body.Bytes(), &e1)
	...
	var e2 entryResp
	_ = json.Unmarshal(rr2.Body.Bytes(), &e2)
	if e1.ID != e2.ID {
		t.Fatalf("idempotency mismatch: %s vs %s", e1.ID, e2.ID)
	}
	...
	if rr3.Code != http.StatusCreated {
		t.Fatalf("expected 201, got %d: %s", rr3.Code, rr3.Body.String())
	}
	var e3 entryResp
	_ = json.Unmarshal(rr3.Body.Bytes(), &e3)
	if e3.ID == e1.ID {
		t.Fatalf("expected new entry without idempotency header")
	}
}
...
func TestEntries_Validation422(t *testing.T) {
	...
	if e.Code != "too_few_lines" {
		t.Fatalf("expected code too_few_lines, got %q", e.Code)
	}
	...
	if e.Code != "invalid_amount" {
		t.Fatalf("expected code invalid_amount, got %q", e.Code)
	}
	...
	if e.Code != "currency_mismatch" {
		t.Fatalf("expected code currency_mismatch, got %q", e.Code)
	}
	...
	_ = json.Unmarshal(rr.Body.Bytes(), &e)
	if e.Code != "unbalanced_entry" {
		t.Fatalf("expected code unbalanced_entry, got %q", e.Code)
	}
}
...
func TestEntries_Pagination(t *testing.T) {
	...
	if rr1.Code != http.StatusOK {
		t.Fatalf("list page1 expected 200, got %d", rr1.Code)
	}
	var p1 struct {
	...
	if len(p1.Items) != 2 || p1.Next == nil {
		t.Fatalf("expected 2 items and cursor; got %+v", p1)
	}
	...
	h.ServeHTTP(rr2, r2)
	if rr2.Code != http.StatusOK {
		t.Fatalf("list page2 expected 200, got %d", rr2.Code)
	}
	var p2 struct {
		Items []entryResp `json:"items"`
		Next  *string     `json:"next_cursor"`
	...
	if len(p2.Items) != 1 || p2.Next != nil {
		t.Fatalf("expected 1 item and no cursor; got %+v", p2)
	}
	if _, ok := firstIDs[p2.Items[0].ID]; ok {
		t.Fatalf("duplicate id across pages")
	}
}
...
func TestNotFound_Standardized(t *testing.T) {
	...
	var e errResp
	_ = json.Unmarshal(rr.Body.Bytes(), &e)
	if e.Error != "not_found" || e.Code != "not_found" {
	...
	h.ServeHTTP(rr, r)
	if rr.Code != http.StatusNotFound {
		t.Fatalf("expected 404 acc, got %d", rr.Code)
	}
	_ = json.Unmarshal(rr.Body.Bytes(), &e)
	if e.Error != "not_found" || e.Code != "not_found" {
		t.Fatalf("unexpected 404 body: %+v", e)
	...
	h.ServeHTTP(rr, r)
	if rr.Code != http.StatusNotFound {
		t.Fatalf("expected 404 bal, got %d", rr.Code)
	}
	// ledger
	...
	h.ServeHTTP(rr, r)
	if rr.Code != http.StatusNotFound {
		t.Fatalf("expected 404 led, got %d", rr.Code)
	}
}
...
func TestLedger_BalanceConsistency(t *testing.T) {
	...
	h.ServeHTTP(rr, rb)
	if rr.Code != http.StatusOK {
		t.Fatalf("balance expected 200, got %d", rr.Code)
	}
	var br struct {
	...
	rlr := httptest.NewRecorder()
	h.ServeHTTP(rlr, rl)
	if rlr.Code != http.StatusOK {
		t.Fatalf("ledger expected 200, got %d", rlr.Code)
	}
	var l struct {
		Items []struct {
	...
	if len(l.Items) == 0 {
		t.Fatalf("expected ledger items")
	}
	if l.Items[len(l.Items)-1].Running != br.BalanceMinor {
		t.Fatalf("running end %d != balance %d", l.Items[len(l.Items)-1].Running, br.BalanceMinor)
	}
}
...
func TestConcurrency_Smoke(t *testing.T) {
	...
	if total < N*M {
		t.Fatalf("expected at least %d entries, got %d", N*M, total)
	}
}
...
func TestAccount_BalanceAndLedger(t *testing.T) {
	...
	rbr := httptest.NewRecorder()
	h.ServeHTTP(rbr, rb)
	if rbr.Code != http.StatusOK {
		t.Fatalf("balance expected 200, got %d", rbr.Code)
	}
	var br struct {
		BalanceMinor int64 `json:"balance_minor"`
	}
	_ = json.Unmarshal(rbr.Body.Bytes(), &br)
	if br.BalanceMinor != 1500 {
		t.Fatalf("unexpected balance: %d", br.BalanceMinor)
	}
	...
	if len(l1.Items) != 1 || l1.NextCursor == nil {
		t.Fatalf("expected 1 item and next_cursor; got: %+v", l1)
	}
}
...
func TestBalance_CurrencyMatchesAccount(t *testing.T) {
	...
	rob := httptest.NewRecorder()
	h.ServeHTTP(rob, rOb)
	if rob.Code != http.StatusOK {
		t.Fatalf("opening-balances expected 200, got %d: %s", rob.Code, rob.Body.String())
	}
	var ob acctResp
	_ = json.Unmarshal(rob.Body.Bytes(), &ob)
	...
	if er.Code != http.StatusCreated {
		t.Fatalf("entry expected 201, got %d: %s", er.Code, er.Body.String())
	}
	...
	h.ServeHTTP(rr, rb)
	if rr.Code != http.StatusOK {
		t.Fatalf("balance expected 200, got %d: %s", rr.Code, rr.Body.String())
	}
	var br struct {
		Currency string `json:"currency"`
	}
	_ = json.Unmarshal(rr.Body.Bytes(), &br)
	if br.Currency != "GBP" {
		t.Fatalf("expected GBP currency in balance, got %s", br.Currency)
	}
	...
	h.ServeHTTP(agrr, agr)
	if agrr.Code != http.StatusOK {
		t.Fatalf("get account expected 200, got %d", agrr.Code)
	}
	var gar acctResp
	_ = json.Unmarshal(agrr.Body.Bytes(), &gar)
	if gar.Currency != "GBP" {
		t.Fatalf("account currency changed unexpectedly: %s", gar.Currency)
	}
}
...
func TestEntries_CurrencyMismatch422(t *testing.T) {
	...
	er := httptest.NewRecorder()
	h.ServeHTTP(er, re)
	if er.Code != http.StatusUnprocessableEntity {
		t.Fatalf("expected 422, got %d: %s", er.Code, er.Body.String())
	}
	var e errResp
	_ = json.Unmarshal(er.Body.Bytes(), &e)
	if e.Code != "currency_mismatch" {
		t.Fatalf("expected currency_mismatch code, got %q", e.Code)
	}
}
...
func TestAccounts_InvalidAndSystemGuards(t *testing.T) {
	...
	if rr.Code != http.StatusBadRequest {
		t.Fatalf("expected 400 for invalid account, got %d", rr.Code)
	}
	...
	rSysRec := httptest.NewRecorder()
	h.ServeHTTP(rSysRec, rSys)
	if rSysRec.Code != http.StatusOK {
		t.Fatalf("opening balances expected 200, got %d: %s", rSysRec.Code, rSysRec.Body.String())
	}
	var sysAR acctResp
	_ = json.Unmarshal(rSysRec.Body.Bytes(), &sysAR)
	...
	h.ServeHTTP(rp, p)
	if rp.Code != http.StatusForbidden {
		t.Fatalf("expected 403 for system account patch, got %d", rp.Code)
	}
	...
	h.ServeHTTP(rd, d)
	if rd.Code != http.StatusForbidden {
		t.Fatalf("expected 403 for system account delete, got %d", rd.Code)
	}
}
...
func TestAccounts_CreateDuplicatePathAndFilters(t *testing.T) {
	...
	rgr := httptest.NewRecorder()
	h.ServeHTTP(rgr, rget)
	if rgr.Code != http.StatusOK {
		t.Fatalf("get account expected 200, got %d: %s", rgr.Code, rgr.Body.String())
	}
	var ga acctResp
	_ = json.Unmarshal(rgr.Body.Bytes(), &ga)
...
func TestEntries_List_PaginationAndFilters(t *testing.T) {
	...
	if rec1.Code != http.StatusOK {
		t.Fatalf("list expected 200, got %d: %s", rec1.Code, rec1.Body.String())
	}
	var page1 struct {
		Items []entryResp `json:"items"`
		Next  *string     `json:"next_cursor"`
