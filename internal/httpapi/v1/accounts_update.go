...
// Allows updating name, group, vendor, and metadata. Enforces immutability on type/currency.
func (s *Server) updateAccount(w http.ResponseWriter, r *http.Request) {
	...
	if payload.Group != nil {
		acc.Group = *payload.Group
	}
	if payload.Vendor != nil {
		acc.Vendor = *payload.Vendor
	}
	...
	if err != nil {
		...
		if errors.Is(err, errs.ErrImmutable) {
			unprocessable(w, "immutable", "immutable")
			return
		}
		if errors.Is(err, errs.ErrUnprocessable) {
			unprocessable(w, "validation_error", "validation_error")
			return
		}
