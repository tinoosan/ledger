...
func (s *Server) getGroupsDictionary(w http.ResponseWriter, r *http.Request) {
    ...
    type groupItem struct{ Type ledger.AccountType `json:"type"`; Groups []dictionary.GroupDef `json:"groups"` }
    out := struct{ Items []groupItem `json:"items"` }{Items: []groupItem{}}
    for _, typ := range types {
        if t != nil && *t != typ { continue }
