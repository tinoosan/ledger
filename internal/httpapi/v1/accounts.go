...
func (s *Server) listAccounts(w http.ResponseWriter, r *http.Request) {
	...
	for _, account := range accounts {
		if query.Name != "" && !equalsFold(account.Name, query.Name) {
			continue
		}
		if query.Currency != "" && !equalsFold(account.Currency, query.Currency) {
			continue
		}
		if query.Group != "" && !equalsFold(account.Group, query.Group) {
			continue
		}
		if query.Vendor != "" && !equalsFold(account.Vendor, query.Vendor) {
			continue
		}
		if query.Type != "" && !equalsFold(string(account.Type), query.Type) {
			continue
		}
		if query.System != nil && account.System != *query.System {
			continue
		}
		if query.Active != nil && account.Active != *query.Active {
			continue
		}
