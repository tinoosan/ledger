...
func (s *Server) postEntriesBatch(w http.ResponseWriter, r *http.Request) {
	...
	if len(req.Entries) == 0 {
		toJSON(w, http.StatusBadRequest, errorResponse{Error: "entries is required"})
		return
	}
	...
	if key := r.Header.Get("Idempotency-Key"); key != "" {
		...
		type normReq struct {
			Entries []normEntry `json:"entries"`
		}
		...
		for _, e := range req.Entries {
			drafts = append(drafts, toEntryDomain(e))
		}
		...
		}{Entries: make([]entryResponse, 0, len(created))}
