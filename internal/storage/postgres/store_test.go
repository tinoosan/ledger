...
func applyInitSQL(t *testing.T, dsn string) {
    ...
    defer cancel()
    s, err := Open(ctx, dsn)
    if err != nil { t.Fatalf("open for init: %v", err) }
    defer s.Close()
...
func truncateAll(t *testing.T, dsn string) {
    ...
    defer cancel()
    s, err := Open(ctx, dsn)
    if err != nil { t.Fatalf("open for truncate: %v", err) }
    defer s.Close()
...
func TestStore_AccountsAndEntries(t *testing.T) {
    ...
    if err := s.Ready(ctx); err != nil { t.Fatalf("ready: %v", err) }

    // Seed dev data to get a user + accounts
    user, accs, err := s.SeedDev(ctx)
    if err != nil { t.Fatalf("seed: %v", err) }
    if user.ID == uuid.Nil || len(accs) < 3 { t.Fatalf("unexpected seed: %+v", user) }

    // Accounts: list + get + update
    list, err := s.ListAccounts(ctx, user.ID)
    if err != nil { t.Fatalf("list accounts: %v", err) }
    if len(list) < 3 { t.Fatalf("expected >=3 accounts, got %d", len(list)) }
    got, err := s.GetAccount(ctx, user.ID, list[0].ID)
    if err != nil { t.Fatalf("get account: %v", err) }
    got.Name = got.Name + " (upd)"
    if _, err := s.UpdateAccount(ctx, got); err != nil { t.Fatalf("update account: %v", err) }

    // Entries: create + list + get + update
    // Use the first two accounts for a balanced entry in GBP
    if len(list) < 2 { t.Fatalf("need at least 2 accounts") }
    a1 := list[0]
    a2 := list[1]
    if a1.Currency != a2.Currency { t.Skip("seed produced different currencies; skipping entry test") }
    // Construct entry
    ...
    if err != nil { t.Fatalf("create entry: %v", err) }
    if created.ID == uuid.Nil || len(created.Lines.ByID) != 2 { t.Fatalf("unexpected created entry: %+v", created) }

    gotE, err := s.GetEntry(ctx, user.ID, created.ID)
    if err != nil { t.Fatalf("get entry: %v", err) }
    if len(gotE.Lines.ByID) != 2 { t.Fatalf("expected 2 lines, got %d", len(gotE.Lines.ByID)) }

    listE, err := s.ListEntries(ctx, user.ID)
    if err != nil { t.Fatalf("list entries: %v", err) }
    if len(listE) < 1 { t.Fatalf("expected >=1 entry") }

    // Update entry (mark reversed)
    gotE.IsReversed = true
    if _, err := s.UpdateJournalEntry(ctx, gotE); err != nil { t.Fatalf("update entry: %v", err) }

    // Idempotency mapping
    key := "test-key-1"
    if err := s.SaveIdempotencyKey(ctx, user.ID, key, created.ID); err != nil { t.Fatalf("save idem: %v", err) }
    if _, ok, err := s.GetEntryByIdempotencyKey(ctx, user.ID, key); err != nil || !ok { t.Fatalf("get idem: %v ok=%v", err, ok) }
}
...
func newBalancedEntry(userID, accDebit, accCredit uuid.UUID, amt money.Amount) ledger.JournalEntry {
    ...
    dID := uuid.New(); cID := uuid.New(); eID := uuid.Nil
