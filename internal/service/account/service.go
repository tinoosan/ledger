...
func (s *service) EnsureOpeningBalanceAccount(ctx context.Context, userID uuid.UUID, currency string) (ledger.Account, error) {
	...
	if err := s.ValidateCreate(a); err != nil {
		return ledger.Account{}, err
	}
...
func (s *service) ValidateCreate(account ledger.Account) error {
	...
	if !slug.IsSlug(strings.ToLower(account.Group)) {
		return errors.New("invalid group slug")
	}
	if account.Vendor == "" {
...
func (s *service) EnsureAccountsBatch(ctx context.Context, userID uuid.UUID, specs []ledger.Account) ([]ledger.Account, []ItemError, error) {
	...
	for _, a := range normalized {
		if _, ok := seenCurr[a.Currency]; ok {
			continue
		}
	...
	if b, ok := s.writer.(txBeginner); ok {
		...
		for _, a := range normalized {
			...
			if acc.Type == ledger.AccountTypeEquity && strings.EqualFold(acc.Group, "opening_balances") {
				acc.Vendor = "System"
				acc.System = true
			}
			if _, err := tx.CreateAccount(ctx, acc); err != nil {
				_ = tx.Rollback(ctx)
				return nil, nil, err
			}
...
func (s *service) Create(ctx context.Context, account ledger.Account) (ledger.Account, error) {
	...
	if _, err := s.EnsureOpeningBalanceAccount(ctx, account.UserID, account.Currency); err != nil {
		return ledger.Account{}, err
	}
	...
	if accNew.Type == ledger.AccountTypeEquity && strings.EqualFold(accNew.Group, "opening_balances") {
		accNew.Vendor = "System"
		accNew.System = true
	}
...
func (s *service) Deactivate(ctx context.Context, userID, accountID uuid.UUID) error {
	...
	acc.Active = false
	if _, err := s.writer.UpdateAccount(ctx, acc); err != nil {
		return err
	}
	return nil
}
